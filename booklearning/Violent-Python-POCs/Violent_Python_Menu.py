...
os.system('cat /root/ArmsCommander/banners/banner_VP.txt')
...
def option_1():
    ...
    cmd_String = "gnome-terminal -e 'bash -c \"sudo python /root/ArmsCommander/booklearning/Violent-Python-POCs/sshBrute.py -H {0} -u {1} -F {2}; exec bash\"'".format(
        target_Host,
        target_Username,
        target_Wordlist
    )
...
def start_credit_card_attack():
    ...
    # /root/ArmsCommander/booklearning/Violent-Python-POCs/wirelessCreditCardsniffer.py
    # tcp_Dump_PCAP = open("/root/ArmsCommander/logs/CreditCardInfocreditcard_Pkt_Capture.pcap", 'w')
    timestr = time.strftime("%Y%m%d-%H%M%S")

    basic_Filename = "/root/ArmsCommander/logs/CreditCardInfo/CreditCard_Pkt_Capture_"
    ...
    cmd_String = "gnome-terminal -e 'bash -c \"sudo python /root/ArmsCommander/booklearning/Violent-Python-POCs/wirelessCreditCardsniffer.py -i {0}; exec bash\"'".format(
        monitor_Interface
    )
...
def option_4():
    ...
    cmd_String = "gnome-terminal -e 'bash -c \"sudo python /root/ArmsCommander/booklearning/Violent-Python-POCs/ftp-sniffer.py -i {0}; exec bash\"'".format(
        network_interface
    )
    print colored(cmd_String, 'red', 'on_white')
    os.system(cmd_String)
...
def option_5():
    ...
    cmd_String = "gnome-terminal -e 'bash -c \"sudo python /root/ArmsCommander/booklearning/Violent-Python-POCs/exifFetch.py -u {0}; exec bash\"'".format(
        target_url
    )
    print colored(cmd_String, 'red', 'on_white')
    os.system(cmd_String)
...
def option_6():
    ...
    cmd_String = "gnome-terminal -e 'bash -c \"sudo python /root/ArmsCommander/booklearning/Violent-Python-POCs/9-mitnickAttack.py -s {0} -S {1} -t {2}; exec bash\"'".format
    (
        source_SYN,
        source_Spoof,
        target_IP
