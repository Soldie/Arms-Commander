...
# Troubleshooting: Pupy

> #1. I am not getting a connection to my remote listener when I test clicking on the payload generated by Pupy

Where exactly did you generate the payload? This matters a lot. Because starting with the April 17th update, Pupy generates UNIQUE SSL keys that are SPECIFIC to the "host machine", or the machine that made the payload.

If you are using a REMOTE server, like Amazon Web Services AWS EC2, then you need to generate the payloads IN THAT SERVER, then move it over to your test box.

You can debug Pupy, and determine if something is failing to authenticate by doing two things:

	1. You can run the TESTER module in ArmsCommander, and double check that tcp_cleartext payloads work, that means Pupy is able to reach you with NO Authentication_File
	2. Secondly, you can run this command on Pupy Server, "logging DEBUG". You will quickly see a wall of text

	>>> INFO:PUPY/443:accepted ::ffff:Victim_IP:35934
DEBUG:PUPY/443:::ffff:Victim_IP:35934 Wait for authentication result
INFO:PUPY/443:::ffff:Victim_IP:35934 failed to authenticate, rejecting connection
DEBUG:PUPY/443:::ffff:Victim_IP:35934 Wait complete: None
DEBUG:PUPY/443:::ffff:Victim_IP:35934 Shutting down

That means that the payload is indeed working and calling back to the host, but it failed to authenticate (the payload has a copy of the key at generation).

Go back, generate your payloads on your remote server, and try it again.

> #2. I tried to add a persistence scriptlet to Pupy but it doesnt do anything upon code execution

Probably because of number 3 below. I wouldn't mess with a persistence scriptlet at all. Not even a keylogger script because it too can raise alarms. At best, I would recommend no more than a PrependMigrate style script or changename (to something like svchost.exe or notepad.exe)

> #3. I tried to manually get persistence but instead the shell "died"

There are no shortcuts in remote exploitation unfortunately. You tripped the Antivirus alarm by messing with the disk way too early. You have to first, build up your privileges, perform internal reconnissance, migrate to a higher level process before ever messing with the registry or dropping a executable file. In other words, steal tokens, migrate, gain privileges, keystroke scan, and then "getsystem".

The AV, and other things like IDS (Intrusion Detection System) is your ENEMY. Have you ever played Metal Gear Solid? You just got Snake killed. Press start to CONTINUE.
